...
const CHECKPOINT_ERROR_MESSAGES = {
...
class SAXParser {
     ...
     */
    resume(checkpointBytes, options) {
        if (!this.wasmSaxParser) {
        ...
        try {
            ...
            const result = resume(pointer, length, this.events ?? 0, Number(consumedOffset & 0xffffffffn), Number(consumedOffset >> 32n));
            if (result !== 0) {
