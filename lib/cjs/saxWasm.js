...
exports.readPosition = exports.readU32 = exports.readCheckpointConsumedBytes = exports.readString = exports.SAXParser = exports.CheckpointError = exports.CHECKPOINT_HEADER_LENGTH = exports.CHECKPOINT_VERSION = exports.Tag = exports.Text = exports.ProcInst = exports.Attribute = exports.Position = exports.Reader = exports.AttributeType = exports.SaxEventType = void 0;
/**
...
exports.CHECKPOINT_HEADER_LENGTH = 42;
/**
...
class SAXParser {
     ...
     */
    resume(checkpoint, expectedConsumedBytes) {
        const wasm = this.wasmSaxParser;
        if (!wasm) {
        ...
        new DataView(memory.buffer).setBigUint64(offsetPointer, BigInt(expectedConsumedBytes ?? (0, exports.readCheckpointConsumedBytes)(checkpoint)), true);
        const result = wasm.restore_checkpoint(checkpointPointer, checkpoint.byteLength, ~~this.events, expectedConsumedBytes === undefined ? 0 : offsetPointer);
        if (result !== 0) {
            throw restoreError(result);
        }
        return (0, exports.readCheckpointConsumedBytes)(checkpoint);
    }
    /**
     ...
     * offset.
     */
    static readCheckpointConsumedBytes(checkpoint) {
        return (0, exports.readCheckpointConsumedBytes)(checkpoint);
    }
...
const checkpointView = (checkpoint, offset, length) => {
 ...
 */
const readCheckpointConsumedBytes = (checkpoint) => {
    return Number(checkpointView(checkpoint, 9, 8).getBigUint64(0, true));
};
exports.readCheckpointConsumedBytes = readCheckpointConsumedBytes;
const restoreErrorMessages = {
    [-1 /* CheckpointErrorCode.InvalidMagic */]: 'Not a sax-wasm checkpoint (bad magic prefix).',
    [-2 /* CheckpointErrorCode.UnsupportedVersion */]: 'Unsupported checkpoint format version.',
    [-3 /* CheckpointErrorCode.Truncated */]: 'Checkpoint is truncated or contains trailing bytes.',
    [-4 /* CheckpointErrorCode.UnsupportedOptions */]: 'Checkpoint uses unsupported parser options.',
    [-5 /* CheckpointErrorCode.InvalidState */]: 'Checkpoint contains an unknown lexical state.',
    [-6 /* CheckpointErrorCode.InvalidAttrType */]: 'Checkpoint contains an unknown attribute type.',
    [-7 /* CheckpointErrorCode.EventMaskMismatch */]: 'Checkpoint event mask does not match this parser.',
    [-8 /* CheckpointErrorCode.ConsumedOffsetMismatch */]: 'Checkpoint consumed offset does not match the expected value.',
};
const restoreError = (code) => new CheckpointError(restoreErrorMessages[code] ?? `Checkpoint restore failed with code ${code}.`, code);
/**
 * Validates everything readable from the fixed header so that a bad
 * checkpoint rejects before touching WebAssembly state.
 */
const validateCheckpointHeader = (checkpoint, events, expectedConsumedBytes) => {
    if (!(checkpoint instanceof Uint8Array)) {
        throw new CheckpointError('Checkpoint must be a Uint8Array.');
    }
    if (checkpoint.byteLength < exports.CHECKPOINT_HEADER_LENGTH) {
        throw restoreError(-3 /* CheckpointErrorCode.Truncated */);
    }
    for (let i = 0; i < 4; i++) {
        if (checkpoint[i] !== [0x53, 0x41, 0x58, 0x43][i]) {
            throw restoreError(-1 /* CheckpointErrorCode.InvalidMagic */);
        }
    }
    const version = checkpoint[4];
    if (version !== exports.CHECKPOINT_VERSION) {
        throw new CheckpointError(`Unsupported checkpoint format version ${version}; this build supports version ${exports.CHECKPOINT_VERSION}.`, -2 /* CheckpointErrorCode.UnsupportedVersion */);
    }
    const options = checkpointView(checkpoint, 5, 2).getUint16(0, true);
    if (options !== 0) {
        throw restoreError(-4 /* CheckpointErrorCode.UnsupportedOptions */);
    }
    const mask = checkpointView(checkpoint, 7, 2).getUint16(0, true);
    if (mask !== (events & 0xffff)) {
        throw restoreError(-7 /* CheckpointErrorCode.EventMaskMismatch */);
    }
    if (expectedConsumedBytes !== undefined) {
        const consumed = (0, exports.readCheckpointConsumedBytes)(checkpoint);
        if (consumed !== expectedConsumedBytes) {
            throw new CheckpointError(`Checkpoint consumed offset is ${consumed}, expected ${expectedConsumedBytes}.`, -8 /* CheckpointErrorCode.ConsumedOffsetMismatch */);
        }
    }
};
